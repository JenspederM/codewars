...
def construct_lookup(r):
    ...
    for a in r.split(','):
        ...
        nbr = a_split[0]
        code = " ".join(a_split[-2:])
        street = " ".join(a_split[1:-2])

        if code not in lookup:
            lookup[code] = {'streets': [street], 'numbers': [nbr]}
        else:
            lookup[code]['streets'].append(street)
            lookup[code]['numbers'].append(nbr)

    return lookup


def travel(r: str, zipcode: str):
    lookup = construct_lookup(r)

    val = lookup.get(zipcode)
    if val is None:
        return f'{zipcode}:/'
    else:
        streets = ','.join(val['streets'])
        numbers = ','.join(val['numbers'])
        return f'{zipcode}:{streets}/{numbers}'
