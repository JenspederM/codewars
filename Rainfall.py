...
def format_data(town, strng: str):
    rain_by_town = strng.splitlines()

    for measurement_string in rain_by_town:
        name, measurement = measurement_string.split(":")
        if name == town:
            return [float(m[3:]) for m in measurement.split(",")]

    return -1.0


def mean(town, strng):
    data = format_data(town, strng)
    if data != -1:
...
def variance(town, strng):
    ...
    if data != -1:
        n = len(data)
        avg = sum(data) / n
        return math.sqrt(sum((m - avg)**2 for m in data)/n)**2
    else:
        return -1
